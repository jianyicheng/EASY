			...
			for(BasicBlock::iterator bi=blk->begin(), bend=blk->end(); bi!=bend; ++bi) {
				...
				if(CallInst *cinst = dyn_cast<CallInst>(inst)) { //We have found a call instruction
					...
					if(calledF == orig) { //We have found a call to our original function!
						...
						callParentBB->getInstList().insert(cinst,pslice_call); //insert pslice_call before cinst
						...
						AllocaInst *orig_call_threadID = new AllocaInst(IntegerType::get(ctx, 32), orig->getName().str() + "_orig_threadID");
						StoreInst *orig_thread_store = new StoreInst(orig_threadID, orig_call_threadID, false);
						callParentBB->getInstList().insert(cinst, orig_call_threadID);
						callParentBB->getInstList().insert(cinst, orig_thread_store);
						//For the pSlice function call
						ConstantInt *pslice_threadID = ConstantInt::get(ctx, APInt(32, StringRef("0"), 10));
						AllocaInst *pslice_call_threadID = new AllocaInst(IntegerType::get(ctx, 32), orig->getName().str() + "_pslice_threadID");
						StoreInst *pslice_thread_store = new StoreInst(pslice_threadID, pslice_call_threadID, false);
						callParentBB->getInstList().insert(pslice_call, pslice_call_threadID);
						callParentBB->getInstList().insert(pslice_call, pslice_thread_store);
