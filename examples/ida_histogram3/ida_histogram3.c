...
void *ida_histogram3(void *threadarg) {
    int i, num;
    ...
    for (i = startidx; i < maxidx; i++) {
        //		printf ("%d\n", input[startidx]);
        num = input_array[indirect_addr[i]];
...
#pragma clang loop unroll(enable) 
    for (i = 0; i < NUM_ACCEL; i++) {
        pthread_create(&threads[i], NULL, ida_histogram3, (void *)&data[i]);
    }
